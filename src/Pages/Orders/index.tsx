  ...
  return (
    <div>
      ...
      <div className="grid_container">
        <div className="grid_container_row wid_97px">
          <div className="user_profile">
            <img src={IMG_user} />
            <p>{summary?.data.gender} - {(new Date().getFullYear()) - Number(summary?.data.birth_date?.substring(0, 4))}</p>
          </div>
        </div>
        <div className="grid_container_row wid_239px">
          <div className="user_contact_details">
            <ul>
              <li><img src={IMG_Prouser} /><div className="content">{Number(summary?.data?.id)}</div>  </li>
              <li><img src={IMG_SmartPhone} className="smart_ph_icn"/><div className="content">{summary?.data?.home_phone}</div>  </li>
              <li><img src={IMG_Appartments} className="wid_12px"/><div className="content">{summary?.data?.mobile_phone}</div>  </li>
              <li><img src={IMG_Home} className="img_home_icn" /><div className="content">{summary?.data?.work_phone}</div>  </li>
              <li><img src={IMG_Arroba} className="image_arroba_icn"/><div className="content">{summary?.data?.email}</div>  </li>
            </ul>
          </div>
        </div>
        <div className="grid_container_row pad_0px">
          <div className="communication_activity_body">
            <div className="title">90-DAY COMMUNICATION ACTIVITY</div>
            <div className="content">
              <div className="card border_right_1_white wid_185px">
        ...
        <div className="grid_container_row sms_carrer wid_225px">
          <div className="communication_activity_body">
            <div className="title">SMS CARRIER STATUS</div>
            <div className="content">
              <div className="card">
                <div className="number">{summary?.data?.carrier_status?.status}</div>
                <div className="data">since   {(Month[new Date(summary?.data?.carrier_status?.since).getMonth()]) + ' ' + (new Date(summary?.data?.carrier_status?.since).getDate()) + ', ' + (new Date(summary?.data?.carrier_status?.since).getFullYear())} </div>
              </div>
            </div>
          </div>
        </div>
        ...
        {tabActive === 1 &&
          <div>
            ...
            <div className="recent_orders">
              ...
              <button className={tabsub_button && subtabActive === 2 ? 'button_active tab_btn mar_lr_20px' : 'tab_btn mar_lr_20px'} onClick={() => isOpenSubTabs(2)}>Errors</button>
